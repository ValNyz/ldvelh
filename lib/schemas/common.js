...
export const SexeSchema = z.enum([SEXES.FEMININ, SEXES.MASCULIN, SEXES.AUTRE]);

export const TypeLieuSchema = z.enum(TYPES_LIEU);

export const TypeArcSchema = z.enum(TYPES_ARC);

export const TypeTransactionSchema = z.enum(TYPES_TRANSACTION);

export const CategorieObjetSchema = z.enum(CATEGORIES_OBJET);

export const EtatObjetSchema = z.enum(ETATS_OBJET);

export const RegulariteSchema = z.enum(REGULARITES);
...
export const InventaireItemSchema = z.object({
	nom: z.string().min(1, "Nom objet requis"),
	categorie: z.string().default('equipement'),
	quantite: z.number().positive().default(1),
	localisation: z.string().default('sur_soi'),
...
export const PnjFrequentSchema = z.object({
	pnj: z.string().min(1),
	regularite: RegulariteSchema.default(REGULARITES[1]),
