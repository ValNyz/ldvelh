...
/**
 * Tracker les vraies corrections de clés (évite les faux positifs)
 */
function trackerCorrectionsCles(propsOriginales, synonymes) {
	const corrections = [];
	for (const oldKey of Object.keys(propsOriginales)) {
		const oldKeyLower = oldKey.toLowerCase().trim();
		const cleCanonique = synonymes[oldKeyLower];
		// Correction seulement si un synonyme existe ET diffère
		if (cleCanonique && cleCanonique !== oldKeyLower) {
			corrections.push(`clé "${oldKey}" → "${cleCanonique}"`);
		}
	}
	return corrections;
}

function preprocess(preprocessFn, schema) {
	return z.union([z.string(), z.number(), z.boolean(), z.array(z.any()), z.null(), z.undefined()])
		.transform((val) => preprocessFn(val))
		.pipe(schema);
}

/**
 * Crée un z.enum() compatible Zod v4 à partir d'un tableau importé
 * Le cast en tuple est nécessaire pour Zod v4
 */
function enumFromArray(arr) {
	return z.enum(/** @type {[string, ...string[]]} */(arr));
}
...
const SEXES_ENUM = enumFromArray(SEXES);
const NIVEAUX_LIEU_ENUM = enumFromArray(NIVEAUX_LIEU);
const TYPES_LIEU_ENUM = enumFromArray(TYPES_LIEU);
const CATEGORIES_OBJET_ENUM = enumFromArray(CATEGORIES_OBJET);
const ETATS_OBJET_ENUM = enumFromArray(ETATS_OBJET);
const LOCALISATIONS_OBJETS_ENUM = enumFromArray(LOCALISATIONS_OBJETS);
const TYPES_ARC_ENUM = enumFromArray(TYPES_ARC);
const ETATS_ARC_ENUM = enumFromArray(ETATS_ARC);
const REGULARITES_ENUM = enumFromArray(REGULARITES);
const TYPES_ENTITES_ENUM = enumFromArray(TYPES_ENTITES);
const TYPES_ORGANISATION_ENUM = enumFromArray(TYPES_ORGANISATION);
const TYPES_RELATIONS_ENUM = enumFromArray(TYPES_RELATIONS);
const CATEGORIES_EVENEMENT_ENUM = enumFromArray(CATEGORIES_EVENEMENT);
const TYPES_EVENEMENT_ENUM = enumFromArray(TYPES_EVENEMENT);
const NIVEAUX_CERTITUDE_ENUM = enumFromArray(NIVEAUX_CERTITUDE);
const FREQUENCES_RECURRENCE_ENUM = enumFromArray(FREQUENCES_RECURRENCE);
...
const VALEURS_CANONIQUES = {
	...
	localisation_objet: {
		'on_person': 'sur_soi', 'equipped': 'sur_soi', 'porte': 'sur_soi', 'porté': 'sur_soi',
		'pocket': 'sur_soi', 'poche': 'sur_soi', 'hand': 'sur_soi', 'main': 'sur_soi',
		'suitcase': 'valise', 'luggage': 'valise', 'bagage': 'valise',
		'bag': 'sac', 'backpack': 'sac', 'sacoche': 'sac',
	},
...
export function verifierCoherenceType(typeDecare, nom, proprietes) {
	if (typeDecare === 'arc_narratif') {
		// Ne jamais corriger le type des arcs narratifs
		return { valide: true, confiance: 100, raisons: ['arc_narratif'] }
	}
...
export const PersonnageProprietesSchema = z.object({
	age: preprocess(extraireNombre, z.number().min(0).max(1000)).optional(),
	sexe: preprocess(
	...
	traits: preprocess(versTableau, z.array(z.string()).default([])),
	interet_romantique: preprocess(versBooleen, z.boolean().default(false)),
	arcs: preprocess(versTableau, z.array(z.string()).default([])),
...
export const LieuProprietesSchema = z.object({
	niveau: preprocess(
		...
		NIVEAUX_LIEU_ENUM).optional(),
	type_lieu: preprocess(
		v => normaliserValeur(v, VALEURS_CANONIQUES.type_lieu),
		TYPES_LIEU_ENUM).optional(),
...
export const ObjetProprietesSchema = z.object({
	type_objet: preprocess(
		v => normaliserValeur(v, VALEURS_CANONIQUES.type_objet),
		CATEGORIES_OBJET_ENUM).optional(),
	quantite: preprocess(extraireNombre, z.number().min(1).default(1)),
	valeur: preprocess(extraireNombre, z.number().min(0)).optional(),
	etat: preprocess(
		v => normaliserValeur(v, VALEURS_CANONIQUES.etat_objet),
		ETATS_OBJET_ENUM.default('bon')
	),
	localisation: preprocess(
		v => normaliserValeur(v, VALEURS_CANONIQUES.localisation_objet),
		LOCALISATIONS_OBJETS_ENUM.default('sur_soi')
	),
	description: z.string().optional(),
});

export const OrganisationProprietesSchema = z.object({
	type_org: TYPES_ORGANISATION_ENUM.optional(),
...
export const ArcNarratifProprietesSchema = z.object({
	type_arc: preprocess(
		...
		TYPES_ARC_ENUM).optional(),
	description: z.string().optional(),
	progression: preprocess(extraireNombre, z.number().min(0).max(100).default(0)),
	etat: preprocess(
		v => normaliserValeur(v, VALEURS_CANONIQUES.etat_arc),
		ETATS_ARC_ENUM.default('actif')
	),
	obstacles: preprocess(versTableau, z.array(z.string()).default([])),
	pnjs_impliques: preprocess(versTableau, z.array(z.string()).default([])),
});
...
export const ProtagonistProprietesSchema = z.object({
	...
	hobbies: preprocess(versTableau, z.array(z.string()).default([])),
...
export function validerEntite(typeDecare, nom, proprietes) {
	...
	// Tracker les corrections de clés (version corrigée)
	corrections.push(...trackerCorrectionsCles(proprietes, synonymes));
...
export function normaliserEntite(typeDecare, nom, proprietes) {
	...
	if (result.erreurs.length > 0) {
		...
		console.warn(`[Schema] ${nom} - Propriétés reçues:`, JSON.stringify(proprietes, null, 2));
	}
...
export const ConnaitProprietesSchema = z.object({
	...
	niveau: preprocess(extraireNombre, z.number().min(0).max(10).default(1)).optional(),
	etape_romantique: preprocess(extraireNombre, z.number().min(0).max(6)).optional(),
	contexte: z.string().optional(),
	depuis_cycle: preprocess(extraireNombre, z.number()).optional(),

	// Propriétés transitoires (utilisées par MODIFIER_RELATION, pas persistées telles quelles)
	niveau_delta: preprocess(extraireNombre, z.number().min(-3).max(1)).optional(),
...
export const PossedeProprietesSchema = z.object({
	quantite: preprocess(extraireNombre, z.number().min(1).default(1)),
	depuis_cycle: preprocess(extraireNombre, z.number()).optional(),
	dernier_cycle_utilise: preprocess(extraireNombre, z.number()).optional(),
});
...
export const FrequenteProprietesSchema = z.object({
	regularite: preprocess(
		v => normaliserValeur(v, { 'often': 'souvent', 'sometimes': 'parfois', 'rarely': 'rarement' }),
		REGULARITES_ENUM.default('parfois')
	),
	periode: z.string().optional(),
});

export const AVisiteProprietesSchema = z.object({
	cycles: preprocess(versTableau, z.array(z.number()).default([])),
}).strict();

export const DoitServiceAProprietesSchema = z.object({
	quoi: z.string(),
	importance: preprocess(extraireNombre, z.number().min(1).max(5).default(2)),
});
...
export function normaliserProprietesRelation(typeRelation, proprietes) {
	...
	if (result.erreurs.length > 0) {
		...
		console.warn(`[Schema] Relation ${typeRelation} - Propriétés reçues:`, JSON.stringify(proprietes, null, 2));
	}
...
export const RecurrenceSchema = z.object({
	frequence: preprocess(
		v => SYNONYMES_EVENEMENTS.frequence[String(v).toLowerCase()] || v,
		FREQUENCES_RECURRENCE_ENUM
	),
	prochain_cycle: preprocess(extraireNombre, z.number()).optional(),
	jour_semaine: z.string().optional(),
	jour_mois: preprocess(extraireNombre, z.number().min(1).max(31)).optional(),
});

export const EvenementSchema = z.object({
	type: preprocess(
		v => SYNONYMES_EVENEMENTS.type[String(v).toLowerCase()] || v,
		TYPES_EVENEMENT_ENUM.default('passe')
	),
	categorie: preprocess(
		v => SYNONYMES_EVENEMENTS.categorie[String(v).toLowerCase()] || v,
		CATEGORIES_EVENEMENT_ENUM.default('social')
	),
	titre: z.string().min(1, "Titre requis"),
	description: z.string().optional(),
	cycle: preprocess(extraireNombre, z.number().min(1)),
	heure: z.string()
	...
	participants: preprocess(versTableau, z.array(z.string()).default([])),
	montant: preprocess(extraireNombre, z.number()).optional(),
});

export const EvenementPlanifieSchema = EvenementSchema.extend({
	type: z.literal('planifie').default('planifie'),
	cycle_prevu: preprocess(extraireNombre, z.number().min(1)),
...
export function normaliserEvenement(data, estPlanifie = false) {
	...
	if (!result.success) {
		...
		console.warn(`[Events] Opération complète:`, JSON.stringify(op, null, 2));
...
const ModifierEntiteSchema = z.object({
	...
	alias_ajouter: preprocess(versTableau, z.array(z.string()).default([])),
	// visible/background pour épistémologie
	visible: z.record(z.string(), z.any()).optional(),
	background: z.record(z.string(), z.any()).optional(),
...
const CreerRelationSchema = z.object({
	...
	type: TYPES_RELATIONS_ENUM,
	proprietes: z.record(z.string(), z.any()).default({}),
	source_type: TYPES_ENTITES_ENUM.optional(),
	cible_type: TYPES_ENTITES_ENUM.optional(),
	certitude: NIVEAUX_CERTITUDE_ENUM.default('certain'),
...
const ModifierRelationSchema = z.object({
	...
	type: TYPES_RELATIONS_ENUM,
	proprietes: z.record(z.string(), z.any()).default({}),
	certitude: NIVEAUX_CERTITUDE_ENUM.optional(),
...
const CreerEvenementSchema = z.object({
	...
	description: z.string().optional(),
	cycle: preprocess(extraireNombre, z.number()).optional(),
	heure: z.string()
		.regex(/^\d{1,2}h\d{2}$/)
		.optional()
		.or(z.literal(''))
...
const PlanifierEvenementSchema = z.object({
	...
	description: z.string().optional(),
	cycle_prevu: preprocess(extraireNombre, z.number().min(1, "cycle_prevu requis")),
	heure: z.string()
		.regex(/^\d{1,2}h\d{2}$/)
		.optional()
		.or(z.literal(''))
