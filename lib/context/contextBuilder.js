 ...
 * Construit le contexte envoyé au LLM pour chaque requête
 */
...
// ============================================================================
// RÉCUPÉRATION ÉVÉNEMENT D'ARRIVÉE
// ============================================================================

/**
 * Récupère l'événement d'arrivée non réalisé (pour le premier LIGHT)
 */
async function getEvenementArrivee(supabase, partieId) {
	const { data, error } = await supabase
		.from('kg_evenements')
		.select(`
			id, titre, description, heure, lieu_id,
	...
	// Récupérer le premier_contact s'il existe
	const { data: participants } = await supabase
		.from('kg_evenement_participants')
		.select(`
			role,
			kg_entites!kg_evenement_participants_entite_id_fkey(nom, type)
		`)
		.eq('evenement_id', data.id)
		.eq('role', 'premier_contact');

	const premierContact = participants?.[0]?.kg_entites?.nom || null;
	...
	return {
		...
		heure: data.heure,
		lieu_nom: data.kg_entites?.nom,
		premier_contact: premierContact
	};
}

/**
 * Marque l'événement d'arrivée comme réalisé
 */
export async function marquerArriveeRealisee(supabase, partieId) {
	...
	if (error) {
		console.error('[Context] Erreur marquage arrivée:', error);
	}
}

// ============================================================================
// RÉCUPÉRATION MESSAGES ET RÉSUMÉS
// ============================================================================
...
/**
 * Formate l'événement d'arrivée pour le premier LIGHT
 */
function formatEvenementArrivee(evt) {
	let output = '=== ÉVÉNEMENT D\'ARRIVÉE ===\n';
	output += '⭐ C\'est le PREMIER narratif de l\'aventure ⭐\n\n';

	// La description contient déjà toutes les instructions formatées
	output += evt.description;
	output += '\n\n';

	if (evt.lieu_nom) {
		output += `Lieu de départ: ${evt.lieu_nom}\n`;
	}
	if (evt.heure) {
		output += `Heure: ${evt.heure}\n`;
	}
...
export async function buildContext(supabase, partieId, gameState, userMessage) {
	...
	// =========================================================================
	// VÉRIFIER SI C'EST LE PREMIER LIGHT (événement arrivée non réalisé)
	// =========================================================================
	const evenementArrivee = await getEvenementArrivee(supabase, partieId);
	const isFirstLight = evenementArrivee !== null;
	...
	const [
		...
		isFirstLight ? Promise.resolve([]) : getResumesCycle(supabase, partieId, cycle),
		isFirstLight ? Promise.resolve([]) : getDerniersMessages(supabase, partieId, CONTEXT_CONFIG.MAX_MESSAGES_RECENTS || 5),
	...
	// PNJ présents (parallèle) - seulement si pas premier light
	let pnjsPresentsData = [];
	if (!isFirstLight && pnjsPresentsNoms.length > 0) {
		pnjsPresentsData = await Promise.all(
	...
	// Événements récents - seulement si pas premier light
	...
	// =========================================================================
	// SI PREMIER LIGHT : Contexte spécial avec événement d'arrivée
	// =========================================================================
	if (isFirstLight) {
		// Événement d'arrivée en premier (instructions pour le narrateur)
		context += formatEvenementArrivee(evenementArrivee);

		// Situation minimale
		context += '=== SITUATION ===\n';
		context += `${gameState?.partie?.jour || ''} ${gameState?.partie?.date_jeu || ''}\n`;
		context += `Lieu: ${lieuActuelNom}\n\n`;

		// Valentin
		context += formatValentin(protagoniste, ia, stats, inventaire, nomAppartement);

		// Lieux disponibles (pour cohérence)
		const lieux = await getEntitesParType(supabase, partieId, 'lieu');
		context += '=== LIEUX DISPONIBLES ===\n';
		for (const lieu of lieux.filter(l => l.proprietes?.niveau !== 'zone')) {
			context += `• ${lieu.nom} (${lieu.proprietes?.type_lieu || '?'})\n`;
		}
		context += '\n';

		// PNJ existants (pour cohérence noms)
		const pnjs = await getEntitesParType(supabase, partieId, 'personnage');
		context += '=== PNJ EXISTANTS ===\n';
		for (const pnj of pnjs) {
			const props = pnj.proprietes || {};
			context += `• ${pnj.nom} — ${props.metier || '?'}`;
			if (props.traits?.length) context += ` (${props.traits.slice(0, 2).join(', ')})`;
			context += '\n';
		}
		context += '\n';
		...
		return {
			context,
			isFirstLight: true,
			evenementArriveeId: evenementArrivee.id,
			protagoniste,
			stats,
			inventaire,
			relationsValentin
	...
	// =========================================================================
	// CONTEXTE NORMAL (pas le premier light)
	// =========================================================================
...
// ============================================================================
// CONTEXTE INIT (inchangé)
// ============================================================================

export function buildContextInit() {
	return 'Nouvelle partie. Génère le monde et tous les éléments initiaux. Tu es le World Builder.';
}
