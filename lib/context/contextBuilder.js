...
// ============================================================================
// FORMATAGE INVENTAIRE ENRICHI (NOUVEAU)
// ============================================================================
...
function formatInventaireEnrichi(inventaire, nomAppartement = 'Appartement') {
	...
	for (const item of inventaire) {
		// Récupérer la localisation depuis relation_props ou directement
		let loc = item.localisation
			|| item.relation_props?.localisation
			|| 'sur_soi';

		// Normaliser les localisations appartement
	...
	for (const loc of localisationsTri) {
		...
		if (!label) {
			// Localisation personnalisée (nom de lieu spécifique)
			label = `📌 ${loc}`;
		}
		// Remplacer "Appartement" par le vrai nom si disponible
		...
		// Grouper par catégorie dans chaque localisation
		const parCategorie = {};
		for (const item of items) {
			const cat = item.categorie
				|| item.objet_props?.categorie
				|| 'autre';
	...
	// Calcul valeur totale (optionnel, pour debug)
...
async function trouverNomAppartement(supabase, partieId) {
	// Chercher le lieu de type habitat où Valentin habite
	...
	// Sinon chercher dans les lieux un qui contient "appartement" dans le nom
...
export async function buildContext(supabase, partieId, gameState, userMessage) {
	...
	if (sceneEnCours?.id) {
		...
		if (messagesScene.length > CONTEXT_CONFIG.MAX_MESSAGES_SCENE) {
			messagesScene = messagesScene.slice(-CONTEXT_CONFIG.MAX_MESSAGES_SCENE);
		}
	}
	...
	// 4. Valentin (avec inventaire enrichi)
