 ...
 * Gère les deux formats : array de strings ou array d'objets
 */
function formatArcs(arcs) {
  ...
  for (const arc of arcs) {
    // Format string : "Titre (type)"
    if (typeof arc === 'string') {
      section += `• ${arc}\n`;
      continue;
    }
    
    // Format objet : { nom, type, progression, etat }
    if (arc.etat && arc.etat !== 'actif') continue;
    
    section += `• ${arc.nom || 'Arc inconnu'} (${arc.type || 'général'}) - ${arc.progression || 0}%\n`;
 ...
 * Gère les lieux en format string ou objet
 */
...
function buildContext({
  ...
  if (recentMessages.length > 0) {
    ...
    context += '(IMPORTANT: Relis ces échanges et maintiens une cohérence ABSOLUE)\n\n';
    // 20 messages pour couvrir ~10 échanges joueur/MJ
    for (const m of recentMessages.slice(-20)) {
      const role = m.role === 'user' ? 'VALENTIN' : 'MJ';
      // Garder suffisamment de contexte par message
