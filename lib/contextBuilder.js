...
function extractLieuActuel(messages, lieux, gameState) {
	// 1. Chercher dans les messages récents (priorité)
	if (messages && messages.length > 0) {
		const dernierAssistant = [...messages].reverse().find(m => m.role === 'assistant');
		if (dernierAssistant) {
			const content = dernierAssistant.content.toLowerCase();

			// Patterns pour détecter les lieux dans le narratif
			const patterns = [
				/(?:arrive[sz]? |entre[sz]? |dans |au |à l[a']|chez )\*?\*?([^*\n,]+)\*?\*?/gi,
				/\*\*([^*]+)\*\*/g  // Lieux en gras dans le narratif
			];

			for (const lieu of (lieux || [])) {
				const nomLieu = lieu.nom?.toLowerCase();
				if (nomLieu && content.includes(nomLieu)) {
					return lieu.nom;
				}
			}
		}
	...
	// 2. Fallback: chercher dans le state si disponible
	// (tu pourrais ajouter un champ lieu_actuel dans le state)
...
async function buildContextForClaude(
	...
	const lieuxFromState = gameState?.lieux || [];
	if (lieuxFromState.length === 0) {
		queries.push(
	...
	let pnjData, messagesData, resumesData, lieuxData;

	if (preloadedPnj) {
		pnjData = preloadedPnj;
		if (lieuxFromState.length === 0) {
			[messagesData, resumesData, lieuxData] = results.map(r => r.data || []);
		} else {
			[messagesData, resumesData] = results.map(r => r.data || []);
			lieuxData = lieuxFromState;
		}
	} else {
		if (lieuxFromState.length === 0) {
			[pnjData, messagesData, resumesData, lieuxData] = results.map(r => r.data || []);
		} else {
			[pnjData, messagesData, resumesData] = results.map(r => r.data || []);
			lieuxData = lieuxFromState;
		}
	}
	...
	const lieuActuel = extractLieuActuel(recentMessages, lieuxData, gameState);
