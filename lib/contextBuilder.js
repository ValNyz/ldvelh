...
function formatSituation(partie, lieu, hierarchieLieu) {
...
function formatPnjPresent(personnage, relation, etats) {
	...
	// Étape romantique
	if (relProps.etape_romantique > 0) {
		const etapes = ['Inconnus', 'Indifférence', 'Reconnaissance', 'Sympathie', 'Curiosité', 'Intérêt', 'Attirance'];
		fiche += `Étape romantique: ${relProps.etape_romantique}/6 (${etapes[relProps.etape_romantique] || '?'})\n`;
	}
...
function formatPnjConnu(personnage, relation) {
	...
	let ligne = `• ${personnage.nom} — rel: ${niveau}/10`;
...
export async function buildContext(supabase, partieId, gameState, userMessage) {
	...
	for (const nom of pnjsPresentsNoms) {
		...
		pnjsPresentsData.push({ personnage, relation, etats });
	}
	...
	context += formatSituation(gameState?.partie, lieuActuel, hierarchieLieu);
	...
	if (pnjsPresentsData.length > 0) {
		...
		for (const { personnage, relation, etats } of pnjsPresentsData) {
