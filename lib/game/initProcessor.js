...
// Stats de base (crédits déterminés dynamiquement)
const STATS_BASE = {
	energie: '3',
	moral: '3',
	sante: '5'
...
export async function processInitMode(supabase, partieId, parsed, cycle = 1) {
	...
	const creditsInitiaux = parsed.valentin?.credits_initiaux
...
async function initValentinStats(supabase, partieId, cycle, creditsInitiaux) {
	...
	const statsInitiales = [
		['energie', STATS_BASE.energie],
		['moral', STATS_BASE.moral],
		['sante', STATS_BASE.sante],
		['credits', String(creditsInitiaux)]
	];
