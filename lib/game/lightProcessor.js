...
export async function processLightMode(supabase, partieId, parsed, cycle, sceneEnCours) {
	...
	// 2. Transactions (MODIFIÉ - support enrichi)
	if (parsed.transactions?.length > 0) {
		const txResults = await processTransactions(supabase, partieId, cycle, parsed);
		resultats.transactions = txResults;
	}
...
/**
 * Traite toutes les transactions d'une réponse
 * Support des nouveaux types : don, deplacement, cadeau_recu, etc.
 */
async function processTransactions(supabase, partieId, cycle, parsed) {
	...
	for (const tx of parsed.transactions) {
		try {
			// Enrichir avec l'heure si disponible
			const transaction = {
				...tx,
				heure: tx.heure || parsed.heure
			};

			const result = await creerTransaction(supabase, partieId, cycle, transaction);
			...
			if (result.success) {
				...
				if (tx.objet) {
					console.log(`[TX] ${tx.type}: ${tx.objet}${tx.quantite > 1 ? ` x${tx.quantite}` : ''}`);
				} else if (tx.montant) {
					console.log(`[TX] ${tx.type}: ${tx.montant > 0 ? '+' : ''}${tx.montant} cr`);
				}
				...
				console.warn(`[TX] Échec ${tx.type}: ${tx.description || tx.objet} - ${result.error}`);
			}
			...
			console.error(`[TX] Erreur ${tx.type}:`, err);
		}
	}

	// Log résumé
	if (resultats.total > 0) {
		console.log(`[TX] Résultat: ${resultats.reussies}/${resultats.total} réussies` +
			(resultats.doublons > 0 ? `, ${resultats.doublons} doublons` : '') +
			(resultats.echouees > 0 ? `, ${resultats.echouees} échouées` : ''));
	}
...
// ============================================================================
// EXTRACTION BACKGROUND (INCHANGÉ)
// ============================================================================
