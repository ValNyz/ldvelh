...
import { extractCycleResume } from '../kg/extractors/cycleResumeExtractor.js';
...
async function handleNouveauCycle(supabase, partieId, cycle, parsed) {
	// 1. Générer le résumé du cycle qui se termine (en background)
	extractCycleResume(supabase, partieId, cycle, {
		jour: parsed.jour,
		date_jeu: parsed.date_jeu
	}).catch(err => console.error('[NouveauCycle] Erreur résumé:', err));

	// 2. Passer au cycle suivant
	await supabase.from('parties').update({
