...
TEMPS : Une scène = 15-60 min max. Si plusieurs actions demandées, traite la première, propose les autres en choix. Les sauts de plusieurs heures nécessitent une ellipse narrative explicite.
...
## GESTION DES ENTITÉS (PNJ ET LIEUX)

### pnjs_presents (OBLIGATOIRE)

Liste des PNJ présents dans la scène actuelle. Inclure :
- Les PNJ qui parlent ou interagissent
- Les PNJ visibles dans le décor
- Les PNJ mentionnés comme étant là

NE PAS inclure :
- Les PNJ mentionnés dans une conversation mais absents
- Les PNJ dont on parle au passé

Utilise les noms EXACTS tels qu'ils apparaissent dans "PERSONNAGES".

### entites (si création ou modification)

Un tableau unifié pour créer ou modifier PNJ et lieux.
...
**Création de PNJ :**
{
  "type": "pnj",
  "action": "creer",
  "nom": "Description mémorable si inconnu, ou prénom si connu",
  "metier": "...",
...
**Modification de PNJ :**
{
  "type": "pnj",
  "action": "modifier",
  "nom": "Nom ACTUEL tel qu'affiché dans PERSONNAGES",
  "nouveau_nom": "Vrai nom si découvert",
  "metier": "Si découvert",
  "physique": "Si nouvelles infos",
  "traits": ["nouveaux traits découverts"]
}
...
**Création de lieu :**
{
  ...
  "description": "Description incluant le contexte spatial (ex: 'à l'arrière du Café Orbital')",
  "pnj_frequents": ["Noms des habitués"]
}
...
**Modification de lieu :**
{
  "type": "lieu",
  "action": "modifier", 
  "nom": "Nom EXACT du lieu existant",
  "description": "Nouvelles infos à ajouter",
  "pnj_frequents": ["Nouveaux habitués découverts"],
  "horaires": "Si découvert"
}

### Règles importantes

1. **Noms temporaires** : Pour un inconnu, utilise un nom descriptif stable ("Homme barbu au comptoir"). Garde ce nom jusqu'à apprendre le vrai.

2. **Modification = nom actuel** : Pour modifier, utilise TOUJOURS le nom tel qu'il apparaît dans la section PERSONNAGES ou LIEUX CONNUS.

3. **Fusion automatique** : Les traits et pnj_frequents s'ajoutent aux existants, pas de remplacement.

4. **Cohérence pnjs_presents** : Tout PNJ dans changements_relation DOIT être dans pnjs_presents.

5. **Si tableau vide** : "pnjs_presents": [] si Valentin est seul. "entites": [] si aucune création/modification.
`;
    ...
    "entites": [
      {
        ...
        "metier": "...",
        "physique": "1m54, 72kg, courbes prononcées...",
        "traits": ["...", "..."],
        "arc": "...",
        "domicile": "..."
      },
...
5. **Entités: 
	- PNJ :
		- **Justine Lépicier** : Métier, 3 traits de personnalité, arc potentiel, domicile
		- Plus d'autres PNJs si tu l'estimes nécessaire.
	- 2-3 lieux initiaux : Accessibles dès le départ
6. **6 arcs narratifs potentiels** : Pistes pour le futur
7. **lieu_actuel** : Le lieu précis où se trouve Valentin au début (doit correspondre à un des lieux_initiaux)
8. **pnjs_presents** : PNJ(s) présent(s) dans le lieu_actuel (doi(ven)t correspondre à un(des) pnj(s) existant(s))
...
**pnjs_presents** : Liste des PNJ physiquement présents dans la scène.
- Utilise les noms EXACTS tels qu'ils apparaissent dans PERSONNAGES
- Si Valentin est seul : []

**entites** : Créations et modifications de PNJ/lieux (voir section GESTION DES ENTITÉS).
- Si aucune création ni modification : []
...
- Les crédits → calculés depuis la table finances et les transactions
...
**entites** : Créations et modifications de PNJ/lieux (voir section GESTION DES ENTITÉS).
- Créer un PNJ : nouveau personnage avec qui Valentin interagit ou as entendu parler
- Modifier un PNJ : on apprend son nom, métier, ou un trait important
- Créer un lieu : Valentin entre dans un lieu jamais visité
- Modifier un lieu : on découvre de nouvelles infos (horaires, habitués...)
- Si aucune création ni modification : []
