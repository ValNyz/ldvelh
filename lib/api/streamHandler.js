	...
	try {
		...
		for await (const event of streamResponse) {
			if (event.type === 'content_block_delta' && event.delta?.text) {
				...
				// Envoyer le chunk brut au client
				await sseWriter.sendChunk(event.delta.text);
...
/**
 * Construit le texte d'affichage depuis un parsed complet
 */
function buildDisplayText(parsed) {
	if (!parsed?.narratif) return null;

	let display = '';

	// Ajouter l'heure
	if (parsed.heure) {
		display += `[${parsed.heure}] `;
	}

	// Nettoyer et ajouter le narratif
	let narratif = parsed.narratif;
	// Enlever l'heure en doublon au début du narratif
	narratif = narratif.replace(/^\[?\d{1,2}h\d{2}\]?\s*[-–—:]?\s*/i, '');
	display += narratif;
