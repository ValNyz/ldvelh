...
/**
 * Récupère la scène précédente (même cycle, numero - 1)
 * ou dernière scène du cycle précédent si première du cycle
 */
export async function getScenePrecedente(supabase, partieId, cycle, numero) {
	let query;

	if (numero > 1) {
		// Scène précédente du même cycle
		query = supabase
			.from('scenes')
			.select('*')
		...
		// Première scène du cycle → dernière scène du cycle précédent
		query = supabase
			.from('scenes')
			.select('*')
			.eq('partie_id', partieId)
