 ...
 * @returns {object} Résultats avec compteurs et erreurs
 */
export async function appliquerOperations(supabase, partieId, operations, cycle) {
	...
	invalidateCacheAfterOperations(partieId, resultats);
...
async function opCreerEvenement(supabase, partieId, op, cycle, resultats) {
	...
	if (op.participants?.length > 0) {
		...
		for (const p of op.participants) {
			...
			if (entiteId) {
				participants.push({ evenement_id: evt.id, entite_id: entiteId, role });
			}
		}
...
function invalidateCacheAfterOperations(partieId, resultats) {
