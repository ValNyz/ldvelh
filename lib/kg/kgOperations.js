...
import { trouverEntite, getEntite, getProtagoniste, getEvenementsAVenir } from './kgService.js';
...
export async function appliquerOperations(supabase, partieId, operations, cycle) {
	...
	for (const op of operations) {
		try {
			// Injecter le cycle dans l'opération pour les fonctions qui en ont besoin
			op._cycle = cycle;
			...
			switch (op.op) {
				...
				case 'MODIFIER_RELATION':
					await opModifierRelation(supabase, partieId, op, cycle, resultats);
					break;
...
async function opCreerEntite(supabase, partieId, op, cycle, resultats) {
	...
	if (op.confirme !== false) {
		await apprendre(supabase, partieId, entiteId, 'nom', op.nom, cycle, 'observation');
...
async function opModifierRelation(supabase, partieId, op, cycle, resultats) {
	...
	// Construire les nouvelles propriétés
	const currentProps = rel.proprietes || {};
	const newProps = { ...currentProps };

	// Gestion du delta de niveau
	if (op.proprietes?.niveau_delta !== undefined) {
		const currentNiveau = currentProps.niveau ?? 0;
		const delta = parseFloat(op.proprietes.niveau_delta) || 0;

		// Appliquer le delta et borner entre 0 et 10
		let newNiveau = currentNiveau + delta;
		newNiveau = Math.round(newNiveau * 10) / 10; // Arrondir à 0.1
		newNiveau = Math.max(0, Math.min(10, newNiveau));

		newProps.niveau = newNiveau;

		if (delta !== 0) {
			console.log(`[Relation] ${op.cible}: ${currentNiveau} → ${newNiveau} (${delta > 0 ? '+' : ''}${delta})`);
		}
	}

	// Gestion de la disposition
	if (op.proprietes?.disposition) {
		newProps.disposition = op.proprietes.disposition;

		// Aussi mettre à jour l'état de l'entité cible
		await supabase.rpc('kg_set_etat', {
			p_partie_id: partieId,
			p_entite_id: cibleId,
			p_attribut: 'disposition',
			p_valeur: op.proprietes.disposition,
			p_cycle: cycle,
			p_details: null,
			p_certitude: 'certain',
			p_verite: true
		...
		resultats.etats_modifies++;
	}

	// Copier les autres propriétés (retirer les champs spéciaux traités ci-dessus)
	if (op.proprietes) {
		// Extraire les champs spéciaux qui ne doivent pas passer par la normalisation
		const { niveau_delta, disposition, ...autresProps } = op.proprietes;

		// Normaliser uniquement les propriétés standard
		if (Object.keys(autresProps).length > 0) {
			const propsNormalisees = normaliserProprietesRelation(op.type, autresProps);
			for (const [key, value] of Object.entries(propsNormalisees || {})) {
				newProps[key] = value;
			}
		}
	}

	const updates = { proprietes: newProps };
...
async function opPlanifierEvenement(supabase, partieId, op, cycle, resultats) {
	...
	const evenementsExistants = await getEvenementsAVenir(
		supabase,
		partieId,
		cycle,
		{
			cycleMin: Math.max(1, (evtNormalise.cycle_prevu || cycle) - 1),
			limit: null,
			skipCache: true
		}
	);
...
function invalidateCacheAfterOperations(partieId, resultats, operations) {
	if (resultats.entites_creees > 0 || resultats.entites_modifiees > 0) {
		cacheInvalidateType(partieId, 'protagoniste');
		cacheInvalidateType(partieId, 'personnages');
		cacheInvalidateType(partieId, 'ia');
		cacheInvalidateType(partieId, 'lieux');
		cacheInvalidateType(partieId, 'objets');
		cacheInvalidateType(partieId, 'connaissances');
...
export async function logExtraction(supabase, partieId, cycle, metrics) {
