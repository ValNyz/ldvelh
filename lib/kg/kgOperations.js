...
async function opCreerEntite(supabase, partieId, op, cycle, resultats) {
	// Normaliser type + propriétés (avec correction automatique du type)
	...
	const { data, error } = await supabase.rpc('kg_upsert_entite', {
		p_partie_id: partieId,
		p_type: type,           // ← Type potentiellement corrigé
		p_nom: op.nom,
		p_alias: op.alias || [],
		p_proprietes: proprietes, // ← Propriétés validées
		p_cycle: cycle,
...
async function opModifierEntite(supabase, partieId, op, resultats) {
	...
	if (op.proprietes) {
		// Normaliser avec le type existant de l'entité
...
async function opCreerRelation(supabase, partieId, op, cycle, resultats) {
	...
	// Création source si manquante (avec détection de type)
	if (!sourceId) {
	...
	// Création cible si manquante (avec détection de type)
	if (!cibleId) {
...
async function opModifierEtat(supabase, partieId, op, cycle, resultats) {
	...
	// Récupérer le type de l'entité pour validation contextuelle
	const entite = await getEntite(supabase, entiteId);

	// Valider et normaliser l'état
	...
	if (validation.correction) {
		console.log(`[Schema] État ${op.attribut}: ${validation.correction}`);
	}

	const { data, error } = await supabase.rpc('kg_set_etat', {
		p_partie_id: partieId,
		p_entite_id: entiteId,
		p_attribut: validation.attribut,  // ← Attribut normalisé
		p_valeur: String(validation.valeur),  // ← Valeur validée
		p_cycle: cycle,
...
async function opPlanifierEvenement(supabase, partieId, op, cycle, resultats) {
	// NOUVEAU : Normaliser l'événement planifié
	const evtNormalise = normaliserEvenement({
		...op,
	...
	console.log(`[KG] Événement planifié: "${evtNormalise.titre}" cycle ${evtNormalise.cycle_prevu}`);
	resultats.evenements_crees++;
	return evt;
}
