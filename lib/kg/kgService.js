...
import { JAUGES } from '../constants.js';
import {
	withCache,
	cacheInvalidate,
	cacheInvalidateType,
...
// ============================================================================
// INVENTAIRE (SANS CACHE) - Vue enrichie
// ============================================================================

export async function getInventaire(supabase, partieId) {
	// PAS DE CACHE pour l'inventaire - il change trop souvent - il bugue surtout
	const { data, error } = await supabase
	...
	if (error) {
		console.error('[KG] Erreur getInventaire:', error);
		return [];
	}
