 ...
 */
export async function runExtractors(supabase, partieId, narratif, cycle, partie, pnjsPresents) {
	...
	try {
		...
		if (entityRelationResult.success) {
			...
			if (ops.length > 0) {
				...
				for (const inv of invalides) {
					...
					console.warn(`[EntityRelation] Opération complète:`, JSON.stringify(op, null, 2));
		...
		await logExtraction(supabase, partieId, cycle, {
			...metrics,
		...
		await logExtraction(supabase, partieId, cycle, metrics);
