...
// ============================================================================
// RÉCUPÈRE LES TRANSACTIONS D'UN CYCLE SPÉCIFIQUE
// ============================================================================
async function getTransactionsCycle(supabase, partieId, cycle) {
	const { data, error } = await supabase
		.from('finances')
		.select('heure, montant, description, objet')
		.eq('partie_id', partieId)
		.eq('cycle', cycle)
		.order('created_at', { ascending: true });

	if (error) {
		console.error('Erreur getTransactionsCycle:', error);
		return [];
	}
